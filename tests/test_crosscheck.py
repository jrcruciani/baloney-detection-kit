...
class TestGenerateInvertedFraming:
    def test_returns_inverted_task(self):
        ...
        assert "<bdk_task>" in args[0][0][0]["content"]
...
class TestRunABTest:
    ...
    def test_comparison_prompt_delimits_untrusted_data(self):
        ...
        assert "<bdk_original_task>" in user_prompt
        assert "<bdk_response_a>" in user_prompt
        assert "<bdk_inverted_task>" in user_prompt
        assert "<bdk_response_b>" in user_prompt
