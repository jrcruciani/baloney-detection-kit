# BDK Behavioral Diagnostics Paper v1.0 — Implementation Plan

> **For the implementing agent:** This plan takes the paper at
> `research/diagnosis-paper/main.tex` (HEAD on `main`, repo `jrcruciani/baloney-detection-kit`)
...
latexmk), `bdk` CLI (`pyproject.toml` already configured),
...
`src/bdk/providers.py` with an `AzureFoundryProvider` before
...
- Vocabulary: keep the "BDK diagnostics / behavioral diagnostics"
...
- License: MIT (see the integrated repository `LICENSE`).
...
- All numbers in §05 and the abstract must be backed by a committed
  artifact under `validation/diagnosis/reproducible/<case>/artifacts/`. No
  hand-typed values.
...
1. `cd` into the BDK repo working copy.
...
6. Open `research/diagnosis-paper/main.pdf` (or just check `ls -la main.pdf` for non-zero
...
**Objective:** Extend `src/bdk/providers.py` with a provider
...
- Modify: `src/bdk/providers.py` (add new class, extend
...
1. Read `src/bdk/providers.py` completely to understand the
...
8. Write the discovered deployments to a new file
   `validation/diagnosis/reproducible/foundry_models.yaml`:
   ```yaml
...
1. Read `validation/diagnosis/reproducible/run_case.py` to confirm the `--runs`
...
3. Run: `python validation/diagnosis/reproducible/run_case.py case-01-sycophancy --runs 5 --out-dir artifacts/runs/`.
4. Aggregate: write a small script
   `validation/diagnosis/reproducible/aggregate_runs.py` that reads `runs/*.json`,
...
1. `python validation/diagnosis/reproducible/run_case.py case-02-host-vs-model --runs 5 --out-dir artifacts/runs/`.
...
1. `python validation/diagnosis/reproducible/run_case.py case-03-ratchet-coherence --runs 10 --out-dir artifacts/runs/`.
...
- Use: `validation/diagnosis/reproducible/cross_judge_case03.py` (already exists)
- Create: `validation/diagnosis/reproducible/case-03-ratchet-coherence/artifacts/cross_judge_agreement.json`
...
3. Run: `python validation/diagnosis/reproducible/cross_judge_case03.py`.
...
1. `gh issue comment 10 --repo jrcruciani/baloney-detection-kit --body "Closed by Case 1 N=5, Case 2 N=5, Case 3 N=10 distribution artifacts committed in paper-v1.0 branch. See validation/diagnosis/reproducible/<case>/artifacts/distribution.json."`
2. `gh issue close 10 --repo jrcruciani/baloney-detection-kit`.
...
> All edits in this workstream are `research/diagnosis-paper/sections/*.tex` or
> `research/diagnosis-paper/appendix/*.tex`. After each task, rebuild with
...
- Modify: `research/diagnosis-paper/sections/05-validation.tex` (replace lines 28–31)
- Read: `validation/diagnosis/reproducible/case-01-sycophancy/artifacts/distribution.json`
...
- Modify: `research/diagnosis-paper/sections/05-validation.tex` lines ~64–110
...
- Modify: `research/diagnosis-paper/appendix/A-prompts.tex`
- Read: `src/bdk/data/prompts.yaml` (single file — confirm path)
  or `src/bdk/data/prompts/*.yaml` (the paper says the latter;
...
- Rewrite: `research/diagnosis-paper/appendix/B-case-transcripts.tex` (full file)
- Modify: `research/diagnosis-paper/appendix/A-prompts.tex` (remove the transcript
...
- Rewrite: `research/diagnosis-paper/appendix/C-code-snippets.tex`
- Modify: `research/diagnosis-paper/appendix/A-prompts.tex` (remove the code TODO block)
- Read: `src/bdk/coherence_llm.py`
...
1. Open a PR: `gh pr create --base main --head paper-v1.0 --title "Paper v1.0" --body-file research/diagnosis-paper/RELEASE_NOTES.md` (create the
...
2. `git tag -a paper-v1.0 -m "BDK Behavioral Diagnostics paper v1.0 — submission-ready preprint"`.
...
1. Confirm Zenodo integration is enabled for the repo:
   ...
   - Toggle `jrcruciani/baloney-detection-kit` to ON (one-time setup; ask
...
2. Verify or create `.zenodo.json` at repo root with metadata.
   **Read the ORCID iD from `.author-identity`** (created in Task 0)
   ...
   ```json
   {
     "title": "Baloney Detection Kit: Behavioral Diagnostics for AI — A Practitioner Toolkit for Post-hoc, User-side Interpretation",
     "description": "Companion preprint and reference implementation for the BDK diagnostic toolkit.",
     "creators": [
     ...
     "related_identifiers": [
       {"identifier": "https://github.com/jrcruciani/baloney-detection-kit", "relation": "isSupplementTo", "scheme": "url"}
     ]
   }
   ```
   Commit on `main` *before* the release: `chore: add .zenodo.json metadata`.
3. Create GH release: `gh release create paper-v1.0 research/diagnosis-paper/main.pdf --title "Paper v1.0" --notes-file research/diagnosis-paper/RELEASE_NOTES.md`.
4. Wait ~5 min for Zenodo webhook to fire. Check
   https://zenodo.org/account/settings/github/repository/jrcruciani/baloney-detection-kit
...
5. **Record the DOI** in `README.md` (add a badge:
   ...
   `research/diagnosis-paper/README.md`. Commit: `docs: add Zenodo DOI badge`.
...
- Create: `research/diagnosis-paper/announcements/microblog.md` (≤300 chars per item,
  multi-post thread style — for impermanente.es via Micro.blog)
- Create: `research/diagnosis-paper/announcements/linkedin.md` (long-form, **no
  hashtags** per user preference)
- Create: `research/diagnosis-paper/announcements/mastodon.md` (≤500 chars, single toot)
...
1. `pdftotext research/diagnosis-paper/main.pdf - | grep -c '\[TODO:'` → `0`.
2. `gh issue list --repo jrcruciani/baloney-detection-kit --state open` no
