...
- **The regex analyzer should be deprecated for anything past a quick-check.** Leaving it as the default in `bdk ratchet` (without `--coherence-judge`) risks exactly the misclassification this case demonstrates. Future work: change the default for multi-step ratchets to warn when the regex analyzer is being used, or auto-invoke the judge when `--output` is requested.
...
python validation/diagnosis/reproducible/run_case.py case-03-ratchet-coherence
```
