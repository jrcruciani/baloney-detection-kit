...
artifacts to validation/diagnosis/reproducible/<case>/artifacts/.

Usage:
    AZURE_FOUNDRY_API_KEY=... AZURE_FOUNDRY_ENDPOINT=... \
        python validation/diagnosis/reproducible/run_case.py <case_dir>
    AZURE_FOUNDRY_API_KEY=... \
        python validation/diagnosis/reproducible/run_case.py <case_dir> --runs 5
...
Each run produces:
    ...
    artifacts/session.json      — full JSON report (via bdk ratchet)
...
from bdk.engine import SYSTEM_PROMPT, DiagnosticEngine
from bdk.labels import parse_labeled_claims
from bdk.providers import create_provider
from bdk.report import generate_json_report, generate_report
...
def run_case_03_ratchet_coherence(
    ...
    from bdk.prompts import get_pure_ratchet_sequence
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="run_case.py",
        description=(
            "Run a reproducible bdk case study against the configured "
            "provider backend."
        ),
    )
    parser.add_argument(
