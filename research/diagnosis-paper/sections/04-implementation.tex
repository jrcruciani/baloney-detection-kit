...
(\texttt{pip install bdk}) with the source code committed to
\url{https://github.com/jrcruciani/baloney-detection-kit}. The architecture
...
The CLI (\texttt{bdk} command, implemented in \texttt{cli.py})
exposes the diagnostic prompts and the orchestration glue. All prompts
live in YAML files under \texttt{src/bdk/data/prompts/}, keyed
by identifier (\texttt{1.1}, \texttt{1.2}, ..., \texttt{4.3}). The
ratchet is run via \texttt{bdk ratchet [--pure] [--behavioral]
...
The repository's \texttt{validation/diagnosis/reproducible/} subtree hosts the
