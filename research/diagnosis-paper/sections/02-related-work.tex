...
work. We summarize how BDK diagnostics relate to each, and what the toolkit
does \emph{not} attempt to do. A more discursive treatment of these
distinctions lives in the repository's \texttt{framework/diagnosis/related-work.md}
document.
...
model~B on task category~$X$?''} BDK does \emph{not}
...
some percentage on safety; BDK helps them understand
...
BDK is diagnostic, not adversarial. Its prompts explicitly
...
or an auditing body. BDK is a \emph{user-side} tool. It does
...
than \emph{accuracy on a benchmark} is foundational to BDK's
approach. Where CheckList is offline and batch-oriented (running suites
of templated test cases against a model), BDK diagnosis is online and
...
white-box access. BDK is strictly behavioral and black-box:
...
\citep{lambert2024rewardbench}. BDK overlaps with this work
